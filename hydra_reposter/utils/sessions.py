...
async def api_get(path: str, *, tries: int = 5) -> dict:
    ...
    for attempt in range(tries):
        ...
        try:
            ...
            console.print(f"[red][LOLZ API ERROR] {e.response.status_code} on {url}[/red]")
            return {}
...
async def fetch_account_info(item_id: int) -> dict:
    ...
    auth_key = _pick(
        tg_raw.get("auth_key"), tg_raw.get("authkey"),
        raw.get("auth_key"), raw.get("authkey")
    )
...
async def login_with_auth_key(item_id: int, sess_path: Path, api_id: int, api_hash: str) -> bool:
    try:
        info = await fetch_account_info(item_id)
    except Exception as e:
        console.print(f"[red][LOLZ API ERROR] Ошибка при получении данных аккаунта {item_id}: {e}[/red]")
        return False
    ...
    try:
        ...
        console.print(f"[red]Ошибка при входе по auth_key для аккаунта {item_id}: {e}[/red]")
    return False

async def login_with_phone_code(item_id: int, sess_path: Path, api_id: int, api_hash: str) -> bool:
    try:
        info = await fetch_account_info(item_id)
    except Exception as e:
        console.print(f"[red][LOLZ API ERROR] Ошибка при получении данных аккаунта {item_id}: {e}[/red]")
    ...
    if not phone:
        console.print(f"[yellow]Отсутствует номер телефона для аккаунта {item_id} ({info.get('title','?')})[/yellow]")
        if getattr(settings, "debug", False):
            console.print(
                f"[blue][DEBUG] Полный info для лота {item_id}: {json.dumps(info, ensure_ascii=False)[:800]}[/blue]"
            )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10) as client_aclose:
            ...
            if resp.status_code != 200:
                console.print(f"[yellow]Код подтверждения не получен for lot {item_id}[/yellow]")
                await client_aclose.aclose()
                await client.disconnect()
                return False
            code = resp.json().get("code")
        if not code:
            console.print(f"[yellow]Не удалось получить код подтверждения для аккаунта {item_id}[/yellow]")
            await client.disconnect()
            return False
        ...
        console.print(f"[red]Ошибка при входе по коду для аккаунта {item_id}: {e}[/red]")
    return False
...
async def run_session_check():
    ...
    for sess_path in sessions_dir.glob("*.session"):
        ...
        try:
            ...
            try:
                if await login_with_auth_key(item_id, sess_path, settings.api_id, settings.api_hash):
                ...
                console.print(f"[red][LOLZ API ERROR] Ошибка при входе через auth_key для аккаунта {item_id}: {e}[/red]")

            try:
                if await login_with_phone_code(item_id, sess_path, settings.api_id, settings.api_hash):
                ...
                console.print(f"[red][LOLZ API ERROR] Ошибка при входе по коду для аккаунта {item_id}: {e}[/red]")
            if getattr(settings, "debug", False):
                try:
