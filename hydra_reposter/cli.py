...
from hydra_reposter.utils.metrics import get_metric, snapshot
...
app = typer.Typer(
    ...
    help="Hydra Reposter — модульная пересылка сообщений в Telegram",
)
console = Console()

DEFAULT_CSV = Path("data/targets.csv")
DEFAULT_DONOR = (
    "https://t.me/+AtkcqZPW5kM1Y2Jl"  # значение по умолчанию – меняется только в коде
)
...
def send(
    ...
    console.print("\n[bold green]Готово![/]")
    console.print(
        f"Отправлено: {get_metric('sent')}, пропущено: {get_metric('skipped')}"
    )


@app.command("check-sessions", help="Проверить авторизацию всех .session")
def check_sessions():
    """Проверяет авторизацию всех .session‑файлов."""
...
def convert(
    tdata: Path = typer.Argument(..., help="Файл tdata"),
...
accounts_app = typer.Typer(
    name="accounts", help="Управление аккаунтами через LolzMarket API"
)
...
def accounts_buy(
    count: int = typer.Option(
        1, "--count", "-c", help="Количество аккаунтов для покупки"
    )
):
    """
    Покупка аккаунтов.
    ...
    async def _buy():
        ...
        async with LolzMarketClient() as client:
                ...
                while success < count and attempts < count * 6:
                    ...
                    try:
                        ...
                        try:
                            ...
                            sess_path = (
                                Path(settings.sessions_dir) / f"{item_id}.session"
                            )
                            sess_path.write_bytes(sess_bytes)
                            console.print(
                                f"[blue]Сохранён .session → {sess_path.name}[/]"
                            )
                            # --- store in DB ---
                            ...
                            console.print(
                                f"[yellow]Не удалось скачать .session:[/] {dl_err}"
                            )
                    except LolzApiError as e:
                        txt = str(e)
                        if "очереди на автоматическую покупку" in txt:
                            console.print(
                                "[yellow]Лот в авто‑очереди, пытаем другой через 1 с…[/]"
                            )
                            await asyncio.sleep(1)
                            continue
                        if "недостаточно средств" in txt:
                            console.print(
                                "[red]Недостаточно средств на балансе Market. Покупка остановлена.[/]"
                            )
                            break
            ...
            if success < count:
                console.print(
                    f"[yellow]Не удалось купить все аккаунты "
        ...
        async with LolzMarketClient() as client:
            ...
            for itm in items:
                ...
                try:
                    ...
                    console.print(
                        f"[blue]Добавлен {sess_path.name} (price {itm['price']})[/]"
                    )
                    with get_session() as db:
                        acc = Account(
                            phone="unknown",
