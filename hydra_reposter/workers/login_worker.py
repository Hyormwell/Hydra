...
async def login_account(session_path: Path, proxy: tuple = None, item_id: int | None = None) -> TelegramClient:
    """
    ...
    if item_id is None:
        ...
        raise RuntimeError(f"Session {session_path.name} не авторизован и нет item_id для получения кода.")
    ...
    try:
        ...
        await client.disconnect()
        raise RuntimeError(f"Не удалось получить код авторизации для {session_path.name}: {e}")
    ...
    if not await client.is_user_authorized():
        await client.disconnect()
        raise RuntimeError(f"Авторизация не выполнена для {session_path.name} даже после ввода кода.")
